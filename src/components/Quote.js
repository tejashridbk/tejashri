...
export default class ValiationForm extends Component {
  state = {enquirydata: {}, quotedata: {} } 

  componentDidMount() {
      console.log(this.props.match.params);
    axios.get('http://localhost:5000/getEnquiries?enquiryId='+this.props.match.params.enquiryId )
    .then(result => this.setState({enquirydata: result.data})); 
    //axios.get('http://localhost:5000/Quote').then(result => this.state.quotedata);   
  ...
  handleSubmit = event => {
    ...
    //if (isValid) {
      axios.post('http://localhost:5000/addQuote', this.state)
  ...
  render() {
      ...
      <form onSubmit={this.handleSubmit}>
        ...
        <div>
           Rate per Kg:<input name="rateperkg" type="number" className="input" value={this.state.rateperkg}
            onChange={this.handleChange}/>
           </div>
           <div>
           Value:<input name="value" type="number" className="input" value={this.state.value}
            onChange={this.handleChange}/>
           </div>
         <div> 
